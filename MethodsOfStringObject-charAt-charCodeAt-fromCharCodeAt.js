...
function topSecret(str){
  var arr = str.split('');
  var charArr = arr.map(x => {
    if(x.charCodeAt() < 100 && x.charCodeAt() > 96) {
      return String.fromCharCode(x.charCodeAt()+23);  
    } else if (x.charCodeAt() < 68 && x.charCodeAt() > 64) {
      return String.fromCharCode(x.charCodeAt()+23);
    } else if (x.charCodeAt() < 123 && x.charCodeAt() > 99) {
      return String.fromCharCode(x.charCodeAt()-3);
    } else if (x.charCodeAt() < 91 && x.charCodeAt() > 67) {
      return String.fromCharCode(x.charCodeAt()-3);
    }
    return x;
  });
